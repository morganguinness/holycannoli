...
export default function HolyCannoli() {
  ...
  const moveDogToClientX = (clientX) => {
    if (!gameRef.current) return;
    const rect = gameRef.current.getBoundingClientRect();
    const rel = clientX - rect.left;
    const clamped = Math.min(Math.max(rel - 40, 0), 400);
    setDogX(clamped);
  };
  ...
  useEffect(() => {
    ...
    const fallInterval = setInterval(() => {
      setPastries((prev) => prev.map((p) => ({ ...p, y: p.y + speed })).filter((p) => p.y < 500));
      setHazards((prev) => prev.map((h) => ({ ...h, y: h.y + speed })).filter((h) => h.y < 500));
      setEmotes((prev) => prev.map((e) => ({ ...e, life: e.life - 0.05 })).filter((e) => e.life > 0));
    }, 50);

...
export default function HolyCannoli() {
  ...
  const resumeOnUserGesture = () => { try { ensureAudio(); } catch {} };
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (!isRunning) return;
      if (e.key === "ArrowLeft") setDogX((prev) => Math.max(prev - 30, 0));
      else if (e.key === "ArrowRight") setDogX((prev) => Math.min(prev + 30, 400));
    };
  ...
  const moveDogToClientX = (clientX: number) => {
    ...
    const clamped = Math.min(Math.max(rel - 40, 0), 400); // ~center the sprite
    setDogX(clamped);
  };
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    if (!isRunning || !isDraggingRef.current) return;
    moveDogToClientX(e.clientX);
  };
  ...
  useEffect(() => {
    ...
    const spawn = setInterval(() => {
      setPastries((prev) => [...prev, { id: `p-${Date.now()}-${Math.random()}`, x: Math.random() * 400, y: 0 }]);
    }, 900);
  ...
  useEffect(() => {
    if (!isRunning) return;
    const spawn = setInterval(() => {
      setHazards((prev) => [...prev, { id: `h-${Date.now()}-${Math.random()}`, x: Math.random() * 400, y: 0 }]);
    }, 1400);
    return () => clearInterval(spawn);
  }, [isRunning]);
    ...
    setPastries((prev: any[]) =>
      prev.filter((p: any) => {
        if (p.y > 420 && p.x > dogX - 30 && p.x < dogX + 80) {
          setScore((s) => s + 1);
          setEmotes((ems) => [...ems, { id: `e-heart-${Date.now()}`, type: "heart", x: dogX + 20, y: 420, life: 1 }]);
          playGulp();
          return false;
        }
        return true;
    ...
    setHazards((prev: any[]) =>
      prev.filter((h: any) => {
        if (h.y > 420 && h.x > dogX - 30 && h.x < dogX + 80) {
          ...
          setEmotes((ems) => [...ems, { id: `e-cry-${Date.now()}`, type: "cry", x: dogX + 20, y: 420, life: 1 }]);
          playVacuum();
          return false;
        }
        return true;
        ...
        {!isRunning && (
          <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center text-white gap-4">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-4 py-2 rounded-full bg-white text-black text-sm hover:bg-neutral-200 transition"
            >
              Follow us
            </a>
          </div>
